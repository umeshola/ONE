...
import { CHEKCK_FRIEND, FRIEND_REQUEST, INFO, LIKE_POST, USER_POST } from '../../connection/query';
export default function SignleUser() {

    const { id } = useParams();
    const variables = {
        data: id
    };
    const { data: d1 } = useQuery(CHEKCK_FRIEND, {
        variables: {
            data: id
        }
    });
    ...
    const { data: d2, refetch } = useQuery(USER_POST, {
        variables
    });
    ...
    const handleFriendRequest = async () => {
        try {
            ...
            console.error('Error sending friend reuest :', error);
        }
    }
    return (
        <div>
            <div className='flex justify-center'>
                <h1 className='text-white font-bold text-3xl'>Post's</h1>
            </div>

            <div className='flex justify-start'>
                <div className=' mt-24 ml-12 mr-16'>
                    <div className=' ml-12 mb-4 flex border-[1px] justify-center items-center border-solid border-white rounded-full w-24 h-24'>
                    ...
                    {d1?.checkfriend ?
                        ...
                        : <div className='mt-10 ml-12'>
                            <button onClick={handleFriendRequest} className="px-2 py-1 bg-gradient-to-r from-red-200 via-blue-100 to-blue-200 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-red-100 dark:focus:ring-red-400 font-bold rounded-lg text-lg text-center me-2">add Friend</button>
                        </div>}
                </div>
                <div className='mt-32 flex flex-wrap'>
                    {d2?.userposts?.length === 0 ? (
                        <div>
